...
import { Trash, ArrowLeft, Save, Calendar, User2, Package, Plus, AlertTriangle } from "lucide-react"
import { type Cliente, type Producto, getClientes, getProductos, addPedido } from "@/lib/data"
import {
  Breadcrumb,
  BreadcrumbItem,
  BreadcrumbLink,
...
export default function NewOrderPage() {
  ...
  const handleAddProduct = () => {
    ...
    const existingIndex = orderDetails.findIndex((item) => item.idProducto === productId)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Ocurrió un error al guardar el pedido")
      setSubmitting(false)
    }
  }
      ...
      <div>
        <Breadcrumb className="mb-4">
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink>Nuevo Pedido</BreadcrumbLink>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 mt-2">Crea un nuevo pedido para un cliente.</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="border-0 shadow-sm lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Completa los datos del pedido y añade los productos.</CardDescription>
          </CardHeader>
        ...
        >
          {product.descripcion} - ${product.precio.toFixed(2)} ({product.cantidad} disponibles)
            ...
            <CardFooter className="flex justify-between border-t pt-6">
              ...
              <Button
                type="submit"
                disabled={submitting || orderDetails.length === 0 || !selectedClient}
                className="gap-2"
              >
                {submitting ? (
                  <>
        ...
        <div className="space-y-6">
          <Card className="border-0 shadow-sm">
            <CardHeader>
              ...
              <CardDescription>Información calculada del pedido actual</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <p className="text-sm font-medium text-slate-500">Total del Pedido</p>
                <p className="text-3xl font-bold mt-1">${calculateSubTotal().toFixed(2)}</p>
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent>
              {selectedClient ? (
                (() => {
                  const client = clients.find((c) => c.idCliente.toString() === selectedClient)
                  return client ? (
                    <div className="space-y-4">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500">Cliente #{client.idCliente}</p>
                        </div>
                      </div>
                ...
                <div className="flex flex-col items-center justify-center py-6 text-slate-500">
                  <User2 className="h-10 w-10 mb-2 text-slate-300" />
                  <p>Ningún cliente seleccionado</p>
                  <p className="text-sm text-slate-400 text-center mt-1">Selecciona un cliente para el pedido</p>
                </div>
              )}
            </CardContent>
          </Card>
